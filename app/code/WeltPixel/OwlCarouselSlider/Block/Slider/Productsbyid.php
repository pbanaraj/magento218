...
class Productsbyid extends \Magento\Catalog\Block\Product\AbstractProduct implements \Magento\Widget\Block\BlockInterface
{
    /**
     * Core registry.
     *
    ...
    )
    {
        ...
        $this->setTemplate('sliders/productsbyid.phtml');
    ...
    public function getProductCollection()
    {
        ...
        $productsId = $this->getData('products_id');
        ...
        switch ($productsType) {
            ...
            case 'products_byid':
                $productCollection =  $this-> _getProductCollectionById($this->_productCollectionFactory->create());
                break;
            case 'new_products':
    ...
    public function getImages($products){
        ...
        $products = $this->productRepository->getById($p[0]['entity_id']);
        
       // $products = $this->productsFactory->create()->load($p[0]['entity_id']);   
      
        $image = $products->getMediaGalleryImages();
    ...
    protected function _getProductCollectionById($_collection)
    {        
        $productsid = $this->getData('products_id');
        $limit  = $this->_getProductLimit('products_byid');
        $random = $this->_getRandomSort('products_byid');
        ...
        ->addAttributeToFilter('entity_id', array('eq' => $productsid));
